...
setwd("~/Desktop/PhD/Projects/Colposcopy CEA/GenotypingCEA/code")
load("~/Desktop/PhD/Projects/Colposcopy CEA/GenotypingCEA/data/ref.data.cens.RData")
source("1_functions.R")
source("2_parameters.R") # ensure parameters 

set.seed(130601)
file.end <- format(Sys.Date(), "%Y%m%d")

########### Create strategy data frame ###########  
...
ascus.strategies <- c("referAll", "referAll", "referAll", "repeat", "repeat", "referAll", "referAll", "refer7", "refer7", "repeat", "repeat", "referAll", "referAll", "refer7", "refer7", "refer1618", "refer1618", "repeat", "repeat")
...
prob_plot <- ggplot(long[long$time!=15,], aes(x=time, y=prob_best, color=best, group=Strategy)) + 
  ...
  theme(legend.position = 'none', plot.margin = unit(c(1,4,1,1), "cm"), text = element_text(size=13)) + 
...
ggsave("~/Desktop/PhD/Projects/Colposcopy CEA/GenotypingCEA/plots/Figure1_NMB.png", dpi=600, width=18, height=10) # save plot 
