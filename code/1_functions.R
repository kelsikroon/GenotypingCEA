...
spec_dec <- function(x, k) trimws(format(round(x, k), nsmall=k)) # function to show k decimal places of x even if there are zeros
...
computeCostsandQALYloss <- function(dat, hpvStrategy, ascusStrategy, normalStrategy, pobParams, probCancerParams)   {
  ...
  CIN3round1 <- (1 - Probcancersdetected_1618)*CIN3round1_1618 + (1 - Probcancersdetected_no1618)*CIN3round1_no1618 # ---> this was (1 - Probcancersdetected_1618)*CIN3round1_no1618 but i changed it
  ...
  QALYloss <- (CIN2round1+CIN3round1)*0.5*LossCIN23 + (CIN2round2+CIN3round2)*0.5*LossCIN23/disce^5
  ...
  QALYloss <- QALYloss +
    ...
    sum(sapply(0:9, function(i) Notdeathfromothercauses5yr*(Survivalyr2[i+1]-Survivalyr2[i+2])*Cancerround2old*0.5*Lossterminal/disce^(i+t_reset + 0.5))) +
    sum(sapply(0:9, function(i) Notdeathfromothercauses5yr*(Survivalyr2[i+1]-Survivalyr2[i+2])*Cancerround2old*(i+0.125)*Losspreterminal/disce^(i+t_reset + 0.125))) +
  ...
  return(c(cost=Cost,qaly=QALYloss, referred = input_data[8], cin2_2rounds = CIN2round1+CIN2round2, cin3_2rounds = CIN3round1 + CIN3round2, cxca_2rounds = Cancerround1 + Cancerround2old + Cancerround2new, CIN2round1)) 
}
  ...
  if (labelLines){
    cea_plot <- cea_plot  +
      geom_text_repel(data=results[results$time_orig == cuttime & results$strategy3!="", ], aes(color = col, label = strategy3),
                      direction = "y", xlim = c(cuttime+0.05, Inf), size=3.5, hjust = 0, segment.size = 0.25, segment.alpha =.25, max.overlaps = 100,
...
survival5yearsLater <- function(time1to23 = 8){
  ...
  load("~/Desktop/PhD/Projects/Colposcopy CEA/GenotypingCEA/data/NKI_data.RData")
