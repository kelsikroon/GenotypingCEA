...
# 3. PROGRESSION PROBABILITY CHANGES FROM 0.5 TO 1.0 ------------------------------------------
source("2_parameters.R") # reset parameters 
cumulativeprogcin3for1618_lifetime <- 1 # update progression parameter
...
source("2_parameters.R")
Costfirsttest <- 53 / 106.16 * 130.31 # update cost of first test: Jansen et al. IJC 2020 --> increased to same as repeat test when considering self-sampling 
...
SA1_plot <- plotCEA(res_SA1, cuttime=10, label='short', labelLines = F, sensPlot = T, maximumY =350)
SA2_plot <- plotCEA(res_SA2, cuttime=10, label='short', labelLines = F, sensPlot = T, maximumY =350)
SA3_plot <- plotCEA(res_SA3, cuttime=10, label='short', labelLines = F, sensPlot = T, maximumY =350)
SA4_plot <- plotCEA(res_SA4, cuttime=10, label='short', labelLines = F, sensPlot = T, maximumY =350)
SA5_plot <- plotCEA(res_SA5, cuttime=10, label='short', labelLines = F, sensPlot = T, maximumY =350)
SA6_plot <- plotCEA(res_SA6, cuttime=10, label='short', labelLines = F, sensPlot = T, maximumY =350)
...
plot_grid(SA1_plot + ggtitle("Time between FIGO 1 and FIGO 2/3 decreased to 4 years"), #A
          ...
          SA3_plot + ggtitle("CIN3 progression probability increased to 1.0"), #C
          SA4_plot + ggtitle("ICER Threshold increased to 20,000EUR"), #D
...
ggsave("~/Desktop/PhD/Projects/Colposcopy CEA/GenotypingCEA/plots/Figure2_NMB.png", dpi = 700, width=20, height=11) # (2200x1200)
